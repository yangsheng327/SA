...
class DSSA:
    ...
    #Prob of low<high is true, return true/false        
    def prob(self, low, high): 
        P_small = 1./(1+np.exp((low-high)/self.k/self.T))#Boltzman distribution 
    ...
    def expand(self):#expansion
        ...
        if self.prob(self.exp_loss, self.ref_loss):
    ...
    def contract(self):
        ...
        if self.prob(self.contr_loss, self.loss[-1]):
...
#            print(self.T)
            self.order()
            if self.prob(self.ref_loss, self.loss[-2]):
                if self.prob(self.loss[0], self.ref_loss):
                    self.reflect()
                else:
                    self.expand()
            else:
...
x0 = np.random.uniform(-20,20, [df + 1, df])
ds = DSSA(loss, x0)
ds.fit(10000)
print(ds.para)
print(ds.loss)
